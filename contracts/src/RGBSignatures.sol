...
contract RGBSignatures is ERC721Enumerable, Ownable {
    ...
    function _generateRandomRGB(
        uint8 salt
    ) internal view returns (uint8 r, uint8 g, uint8 b) {
        uint256 random = uint256(
            keccak256(abi.encodePacked(block.prevrandao, msg.sender, salt))
        );
        r = uint8(random % 256);
        ...
        for (uint8 row = 0; row < 5; row++) {
            for (uint8 col = 0; col < 5; col++) {
                ...
                result = string.concat(
                    result,
                    _renderSquare(row, col, color, centerColor)
                );
            }
        }
